...
decays = [0.995, 0.999, 0.9995, 0.9999]
starts = [0, 1000, 2000]
names = []
...
for lr in learning_rates
    for decay in decays
        for start in starts
            name = "hypertest3_lr_" * string(lr) * "_decay_" * string(decay) * "_start_" * string(start)
            push!(names, name)
            hyper = make_hyper([32, 16, 8, 4],fill(elu,5),6000,32,Flux.NADAM,lr,decay,start,name,"random")
